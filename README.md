...
実際の適用量は、基本的には `(入力値 / 1000) × 適用時点で取得できる現在のフォントサイズ` で計算されます。
ここでいう「現在のフォントサイズ」はベースのフォントサイズに固定された値ではなく、実行時に取得できる有効なサイズ情報（環境によっては `obj.zoom` など）を指します。
フォントサイズを取得できない環境では、後方互換のため 1:1 とみなして入力値がそのまま適用されます。
`manual_font_size` と併用する場合は、文字ごとのサイズ変更を先に反映した上でカーニング量を決められるよう、`manual_font_size` を `manual_kerning` より前に適用することを推奨します。

> **注意:** `manual_font_size` と組み合わせて使用する場合は、アニメーション効果の適用順を **`manual_kerning` → `manual_font_size`** の順（`manual_kerning` を先）にしてください。`manual_font_size` を先に（`manual_font_size` → `manual_kerning` の順に）適用すると、`manual_font_size` が文字ごとに変更した `obj.zoom` を `manual_kerning` が読み取ってしまい、em 換算値が文字ごとに異なって間隔が崩れます。
...
- `manual_kerning` と `manual_font_size` を併用する場合、アニメーション効果の適用順を **`manual_kerning` → `manual_font_size`** の順にしてください。`manual_font_size` を先に適用すると em スケールが文字ごとに変動し、文字間隔が意図通りに調整されません。
